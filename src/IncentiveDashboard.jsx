 ...
 * Adds a dated Activity Log under each card (amount, note, timestamp).
 *
...
export default function MultiIncentiveDashboard() {
  ...
  const handleAddContribution = async (incentive) => {
    ...
    // Optimistic UI
    const optimistic = { id: `local-${Date.now()}`, incentive_id: incentive.id, amount: n, note, created_at: new Date().toISOString() };
                  ...
                  <div style={styles.activity}>
                    ...
                    {list.map((c) => (
                      <div key={c.id} style={styles.activityItem} title={c.note || ""}>
                        <span style={styles.activityAmount}>+{fmt(c.amount)}</span>
