...
/**
 * MULTI‑INCENTIVE DASHBOARD — React + Supabase (No Tailwind)
 * This version uses plain CSS‑in‑JS styles so it looks crisp without Tailwind.
 *
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <header style={styles.header}>
          ...
          {!supabase && showDemoBanner && (
            <div style={{ fontSize: 12, color: "#687481", display: "flex", alignItems: "center", gap: 8 }}>
              <RefreshCw size={14}/> Demo mode (no Supabase keys)
              <button onClick={()=>setShowDemoBanner(false)} style={{ marginLeft: 8, border: "1px solid #E5E7EB", background: "#fff", padding: "4px 8px", borderRadius: 8, cursor: "pointer" }}>Hide</button>
            </div>
          )}
        </header>
        ...
        <div style={{ ...styles.panel, marginBottom: 16 }}>
          ...
          {!supabase && (
            <p style={{ ...styles.sub, marginTop: 8 }}>Demo mode uses your browser storage. Add your Supabase keys to go live.</p>
          )}
        </div>
        ...
        <AnimatePresence>
          {error && (
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} style={styles.error}>{error}</motion.div>
          )}
        </AnimatePresence>
