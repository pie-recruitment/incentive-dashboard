...
const signFmt = (n, money = false) =>
  money ? (n >= 0 ? `+${fmtGBP(n)}` : `-${fmtGBP(Math.abs(n))}`) : n >= 0 ? `+${fmt(n)}` : `-${fmt(Math.abs(n))}`;
...
export default function MultiIncentiveDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        if (!supabase) {
          // Demo fallback so UI renders if env vars missing
          const demoIncs = [
  ...
  // Realtime: INSERT updates instantly; DELETE triggers a refresh
  useEffect(() => {
    if (!supabase) return;
    const ch = supabase
  ...
  async function addOnly(incentive) {
    ...
    const { error } = await supabase
      .from("contributions")
      .insert({ incentive_id: incentive.id, amount: val, note });
    if (error) {
      // rollback
      setTotals((t) => ({ ...t, [incentive.id]: (t[incentive.id] || 0) - val }));
      setLogs((lg) => ({
        ...lg,
        [incentive.id]: (lg[incentive.id] || []).filter((r) => r.id !== temp.id),
      }));
      alert(error.message);
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f7f7f8", padding: 24, color: "#232D34" }}>
          ...
          {[...activityIncs, ...(sales2 ? [sales2] : [])].map((inc) => (
            ...
            >
              ...
              <div style={{ display: "grid", placeItems: "center" }}>
                <Donut
                  percent={Math.min(100, ((totals[inc.id] || 0) / (inc.target || 1)) * 100)}
                />
              </div>
              ...
              <div style={{ borderTop: "1px solid #F2F4F7", paddingTop: 8, maxHeight: 140, overflow: "auto" }}>
                <div style={{ fontSize: 12, color: "#667085", fontWeight: 600, marginBottom: 6 }}>
                  Recent activity
                </div>
                <AnimatePresence>
                  {(logs[inc.id] || []).map((l) => (
                    ...
                    >
                      <span
                        style={{
                          fontWeight: 700,
                          color: l.amount >= 0 ? "#047857" : "#B42318",
                        }}
                      >
      ...
      <style>
        {`@keyframes spin{to{transform:rotate(360deg)}} button{cursor:pointer} button:disabled{opacity:.6;cursor:not-allowed}`}
      </style>
    </div>
  );
}
    ...
    >
      ...
      <div>
        ...
        <div style={{ marginTop: 4, fontWeight: 700 }}>
          {money ? fmtGBP(Math.max(target - total, 0)) : fmt(Math.max(target - total, 0))} remaining
        </div>
      </div>
    </div>
  );
